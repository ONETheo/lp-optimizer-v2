...
class BacktestEngine:
    ...
    def __init__(
        ...
        try:
            ...
            # Uniswap V3 fee tiers are typically 100, 500, 3000, 10000 bps
            self.fee_tier_bps = int(self.pool_details.get('feeTier', 0))
            if self.fee_tier_bps <= 0:
                 ...
                 self.fee_tier_decimal = self.fee_tier_bps / 10000 / 100 # Convert bps to decimal (e.g., 500 -> 0.0005)
            logger.info(f"Using fee tier: {self.fee_tier_bps} bps ({self.fee_tier_decimal:.4%})")
        except ValueError:
             logger.error(f"Invalid feeTier value in pool_details: {self.pool_details.get('feeTier')}")
             raise ValueError("Invalid feeTier value.")
    ...
    def run_vfat_backtest(
        ...
        for i, row in self.historical_data.iterrows():
            ...
            if in_position:
                days_in_position += 1
                # --- Fee Calculation ---
                # This is a significant simplification. Real fees depend on:
                # 1. Actual liquidity provided by the position at the current price tick.
                # 2. Total liquidity in the pool at that tick.
                # 3. Volume traded within the position's range.
                
                # First, estimate what percentage of the pool our position represents
                # Use TVL to determine our portion of the pool
                position_portion = safe_divide(position_value, daily_tvl) 
                
                # Apply a maximum cap on our position's portion of the pool (realistic limit)
                position_portion = min(position_portion, 0.01)  # Cap at 1% of pool

                # Concentration factor based on width - narrower positions provide more concentrated liquidity
                if initial_width_pct > 0:
                    # More conservative concentration estimate:
                    # 1. Start with theoretical maximum: 1/width
                    # 2. Apply diminishing returns using sqrt to account for real-world inefficiencies
                    # 3. Apply a realistic cap
                    theoretical_max = 1.0 / (initial_width_pct / 100.0)
                    concentration_factor = min(np.sqrt(theoretical_max), 20.0)  # Cap at 20x, use sqrt for diminishing returns
                else:
                    concentration_factor = 1.0  # Avoid division by zero
                
                # Calculate daily fees as: Pool Volume * Pool Fee Rate * Our Position Portion * Concentration
                daily_fees = daily_volume * self.fee_tier_decimal * position_portion * concentration_factor
                
                # Apply a maximum fee cap as a sanity check (e.g., max 1% daily return)
                max_daily_fee = position_value * 0.01  # 1% of position value
                daily_fees = min(daily_fees, max_daily_fee)
                
                cumulative_fees_earned += daily_fees
                
                # Log warning about fee calculation once
                if i == 0:  # Only log on first iteration
                    logger.warning("Fee calculation uses a simplified model and may not accurately reflect real Uniswap V3 fee mechanics.")
            ...
            if rebalance_triggered and not rebalance_cutoff:
                rebalance_count += 1
            ...
            results_log.append({
                ...
                "rebalanced": rebalance_triggered and not rebalance_cutoff,
                "rebalance_cutoff": rebalance_cutoff,
                "gas_cost": gas_cost_today,
                "slippage_cost": slippage_cost_today,
                "position_value": position_value # Track value *after* costs for the day
            })
